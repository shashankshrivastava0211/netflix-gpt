...
const Login = () => {
    const [isSignInForm , setisSignInForm] = useState(true);//login
    const toggleSignIn =()=>{
    ...
    return (
        <div>
            ...
            <form className='w-3/12 absolute p-12 bg-black my-36 mx-auto right-0 left-0 text-white bg-opacity-80'>
                ...
                <input type='text' placeholder='Email Adress' className='p-4 my-4 w-full bg-gray-800'/>
               
                <input type='password'placeholder='password' className='p-4 my-4 w-full bg-gray-800'/>
                <button className='p-4 my-6 bg-red-700 w-full mr-2 rounded-2xl'>{isSignInForm ? "Sign In" : "Sign Up"}</button>
